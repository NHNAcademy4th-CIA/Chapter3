...
public class Problem6 {
    ...
    public static void problem6() {
        ...
        for (int i = 2; i <= 10000; i++) {
            ...
            if (!list.contains(i)) {
                num = checkDivisor(list, i); // 넣지 말고 최대값 뽑아서 높은거 있으면 clear하고 다시 넣어 , 같은거면 그냥 넣기
        ...
        for (int i = 0; i < answer.size(); i++) {
            ...
            if (flag.getDivisorNumber() == answer.get(i).getDivisorNumber()) {
                logger.info("가장 많은 약수를 가진 정수 {} , 약수의 개수는 {}", answer.get(i).getDivisorNumber(),
                        answer.get(i).getNumber());
            } else {
                return;
            }
        }
